/*! 
; this is a little modification of TillaGoto (December 30, 2010)
more details about TillaGoto - please look here: http://www.autohotkey.com/forum/viewtopic.php?t=41575
many thanks to Tilla ! :) great! thanks !! :)
*/
;<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
uGotoDef =  ^b
Last_A_This= %uGotoDef% := uGotoDef `n %uGoBack% := uGoBack `n %uGoForward% := uGoForward `n %uGotoDef% := uGotoDef `n %uSummonGUI% := uSummonGUI `n
if(false)
   ToolTip1sec(A_LineNumber . " " . A_ScriptName . " " . Last_A_This)
lll(A_LineNumber, "SL5_AHK_Refactor_engine_v0.5.ahk",Last_A_This)
...
Return
PreviousView:
; works great inside on file but not beetween files 15-06-11_18-33
Last_A_This:=A_ThisFunc . A_ThisLabel
ToolTip1sec(A_LineNumber . " " . A_ScriptName . " " . Last_A_This)
LineHistory(False)
Return
...
; catching up down arrows and mouse wheel
...
Return True
} Else If (wParam = 36) And GetKeyState("Ctrl", "P") {

   ;Ctrl+Home
   ControlSend,, {Home}, ahk_id %hlblList%
   Return True
}
} Else If (hwnd = hlblList) {

   ;Make up/down wrap around
   If (wParam = 38) Or (wParam = 40)
   Return WrapSel(wParam = 38) ? True : ""
}
} Else If (msg = 522) And (hwnd = htxtSearch) {
...
If (iIncludeMode & 0x00000001) {
   ...
   Loop {
      ...
      If InStr(s, "D") {
         ...
         ;It's a folder. Change working directory
         sWorkDir := t
         SetWorkingDir, %sWorkDir%
...
CacheFile(sType, iStart, iStop, iCacheIndex) {
   ...
   While Not iRand {
      ...
      ;Loop until we have a unique number
...
GetScriptHotkeys(ByRef s, bExternal = False) {
   ...
   Loop {
      ...
      If Not IsValidHotkey(t) {
      
         ;It failed validity test. Check if it's the exception [escapechar][commentflag]
...
AppendSymbol(i) {
   ...
   if(false)
      ToolTip1sec(A_LineNumber . " " . A_ScriptName . " " . Last_A_This)
   ; lll(A_LineNumber, "SL5_AHK_Refactor_engine_v0.5.ahk",Last_A_This)
   ;Use \ for include files and | for library functions
...
GetLongestItem(hLB) {
   ...
   if(false)
      ToolTip1sec(A_LineNumber . " " . A_ScriptName . " " . Last_A_This)
   ; lll(A_LineNumber, "SL5_AHK_Refactor_engine_v0.5.ahk",Last_A_This)
   ;We need the listbox to get the font used
   ...
   If (iIncludeMode & 0x10000000) {
      ...
      ;Check if we're taking from the _List elements. Split for speed
      Loop %sLabels0% {
...
LineHistory(bForward, iRecordMode = 0) {
   ...
   Else If (iRecordMode = 2) {
   
      ;Record to the next line
      iCurLine += 1
      LH_GetCurLine(iLines%iCurLine%_%iCurFile%)
